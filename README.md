...
Skill 会引导 Agent 完成：内容契约 → 分页叙事 → 视觉解释 → JSON spec → 本地渲染 → 自动检查 → 人工视觉 QA。
...
渲染 PNG 需要本地 Chromium 或 Playwright；仅做 JSON 结构检查不需要浏览器。
